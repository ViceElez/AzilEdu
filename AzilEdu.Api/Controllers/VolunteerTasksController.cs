...
public class VolunteerTasksController : ControllerBase
{
        ...
        )
    {
        ...
        {
            query = query.Where(task => task.DueDate >= dueDateFrom.Value);
        }

        if (dueDateTo.HasValue)
        {
            query = query.Where(task => task.DueDate <= dueDateTo.Value);
        }
        ...
        if (expired.HasValue)
        {
            if (expired.Value)
            {
                query = query.Where(task => task.DueDate < DateTime.Now);
            }
            else
            {
                query = query.Where(task => task.DueDate >= DateTime.Now || task.CompletedAt != null);
            }
        }
